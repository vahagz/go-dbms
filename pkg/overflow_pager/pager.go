...
func (p *Pager) Marshal(id int, v OverflowMarshaler) error {
	...
	err = p.Write(id, d[0:fpEnd])
	if err != nil {
		return err
	}

	for i, oID := range v.Overflows() {
		from := (i + 1)*p.pageSize
		to := from + p.pageSize
		if to > len(d) {
			to = len(d)
...
// 'into'.
func (p *Pager) Unmarshal(id int, into OverflowUnmarshaler) error {
	d, err := p.Read(id)
	if err != nil {
		return err
	}
